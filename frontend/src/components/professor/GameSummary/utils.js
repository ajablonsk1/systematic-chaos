...
export const GradesTableContent = (tableType, row, idx) => {
  let tableContent = {
    ...
    points: tableType === GradesTableType.GRADES_TABLE ? row.points + '/' + row.maxPoints : row.maxPoints,
...
export const GradesTableHeaders = (tableType) => {
  return [
    'Nr',
    'Aktywność',
    ...(tableType === GradesTableType.GRADES_TABLE ? ['Ocena', 'Punkty'] : ['Punkty do zdobycia']),
    'Data',
    'Typ aktywności'
  ]
}
