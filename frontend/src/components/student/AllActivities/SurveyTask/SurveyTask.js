...
import { useGetSurveyQuery } from '../../../../api/hooks/surveyController.hooks'

export default function FeedbackTask() {
  const location = useLocation()
  const { activityId: taskId } = location.state

  const surveyData = useGetSurveyQuery(taskId)

  return (
    <Content>
      <InfoContainer>
        {surveyData.isFetching ? (
          <Loader />
        ) : surveyData.isError ? (
          <p>{surveyData.errorInfo}</p>
        ) : (
          surveyData.data && (
            <ActivityCol className='invisible-scroll'>
              <HeaderCol>
                <HeaderRow>
                  ...
                  <ActivityName>{surveyData.data.name}</ActivityName>
                </HeaderRow>
                <FullDivider />
                <div>
                  <h5>{surveyData.data.description}</h5>
                  <SmallDivider />
                  <h5>Punkty do zdobycia: {surveyData.data.experience}</h5>
                  <p>
                  ...
                  {({ isSubmitting, values, errors, handleSubmit }) => (
                    <Form onSubmit={handleSubmit}>
                      <Container>
                        ...
                        <p>
                          Jakie są Twoje wrażenia z tego rodziału? Co można zmienić, poprawić, a co było w porządku?
                        </p>
