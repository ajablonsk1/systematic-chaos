...
export default function ActivityContent(props) {
    ...
    useEffect(() => {
        if (loadedScore){
            StudentService.getUserGroup().then(activityGroup => {
                const givenTimeData = props.activity
                    .requirement
                    ?.accessDates
                    .find(el => el.group.find(el => el.name === activityGroup.name));
    ...
    const resetStorageAndStart = () => {
        ...
        const navigateTo = (nodeId, taskResultId) => navigate(`${PageRoutes.QUESTION_SELECTION}`, {
            state: {
    ...
    return (
        <>
            {!loadedScore ? <Spinner animation={"border"}/> : (
